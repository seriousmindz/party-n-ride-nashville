...
function BookingModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void; defaultPackage?: string }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 sm:p-4" data-testid="booking-modal">
      ...
      <div className="relative bg-void-800 shadow-2xl w-full h-full sm:max-w-4xl sm:h-[90vh] sm:rounded-lg border-0 sm:border border-neutral-700 flex flex-col overflow-hidden sm:-skew-x-6">
        <div className="sm:skew-x-6 flex flex-col h-full">
          <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-neutral-700 shrink-0">
            <h3 className="text-lg sm:text-2xl font-display font-bold text-white uppercase tracking-widest">Book Your Ride</h3>
            <button onClick={onClose} className="text-neutral-400 hover:text-crimson-600 text-3xl font-bold transition-colors p-1 min-w-[44px] min-h-[44px] flex items-center justify-center" data-testid="button-close-modal">&times;</button>
          </div>
          <iframe
            src={FAREHARBOR_URL}
    ...
    <nav className="fixed top-0 w-full z-50 bg-transparent backdrop-blur-sm border-b border-white/10">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8">
        <div className="flex justify-between h-14 sm:h-16 items-center">
          ...
          <button className="lg:hidden text-white text-3xl min-w-[44px] min-h-[44px] flex items-center justify-center" onClick={() => setMobileMenuOpen(!mobileMenuOpen)} data-testid="button-mobile-menu">
            {mobileMenuOpen ? '\u2715' : '\u2630'}
          </button>
        </div>
      </div>
      {mobileMenuOpen && (
        <div className="lg:hidden bg-black/60 backdrop-blur-md border-t border-white/10 px-4 py-3 space-y-1 max-h-[80vh] overflow-y-auto">
          {navItems.map(item => (
            <button key={item.path} onClick={() => { navigate(item.path); setMobileMenuOpen(false); }} className={`block w-full text-left font-display font-bold uppercase tracking-wider text-sm py-3 px-3 transition-all duration-200 min-h-[44px] ${currentPage === item.path ? 'text-crimson-600 border-l-2 border-crimson-600 bg-white/5' : 'text-neutral-300 hover:text-white hover:bg-white/5'}`}>{item.label}</button>
          ))}
...
function Footer() {
  return (
    <footer className="w-full bg-void-900/80 border-t border-neutral-800 py-2 px-3 sm:px-6 text-center z-20 shrink-0">
      <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-1 sm:gap-4">
        ...
        <div className="flex items-center gap-2 sm:gap-4 flex-wrap justify-center">
          <a href="https://www.theparkingguys.com" target="_blank" rel="noopener noreferrer" className="text-neutral-500 hover:text-white text-[10px] sm:text-xs font-display tracking-wide transition-colors min-h-[44px] flex items-center px-1" data-testid="link-footer-parkingguys">
            The Parking Guys
          </a>
          <span className="text-neutral-700 text-[10px]">|</span>
          <a href="https://seriousmindz.ai" target="_blank" rel="noopener noreferrer" className="text-neutral-500 hover:text-white text-[10px] sm:text-xs font-display tracking-wide transition-colors min-h-[44px] flex items-center px-1" data-testid="link-footer-seriousmindz-main">
            seriousmindz.ai
          </a>
          <span className="text-neutral-700 text-[10px]">|</span>
          <a href="https://api.seriousmindz.ai" target="_blank" rel="noopener noreferrer" className="text-neutral-500 hover:text-white text-[10px] sm:text-xs font-display tracking-wide transition-colors min-h-[44px] flex items-center px-1" data-testid="link-footer-seriousmindz-api">
            api.seriousmindz.ai
          </a>
        </div>
      </div>
...
function SectionHeader({ tag, title, titleAccent, subtitle }: { tag: string; title: string; titleAccent: string; subtitle: string }) {
  return (
    <div className="mb-4 sm:mb-10 border-l-4 border-crimson-600 pl-3 sm:pl-6 py-2">
      ...
      <h1 className="text-xl sm:text-4xl md:text-5xl font-display font-bold text-white tracking-tighter uppercase mb-2">
        {title} <span className="text-transparent bg-clip-text bg-gradient-to-r from-crimson-600 to-crimson-900">{titleAccent}</span>
      </h1>
      <p className="text-xs sm:text-base md:text-lg text-neutral-500 max-w-2xl font-medium tracking-wide">{subtitle}</p>
    </div>
  );
}
        ...
        <div className="w-full max-w-7xl flex flex-col lg:flex-row items-center justify-between gap-6 sm:gap-8">
          <div className="w-full lg:w-1/2 text-center lg:text-left">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 sm:gap-5 justify-center lg:justify-start">
              ...
              <a href="tel:6153374342" className="text-base sm:text-lg md:text-xl font-display font-bold text-crimson-600 hover:text-white transition-colors min-h-[44px] flex items-center" data-testid="link-hero-phone">
                Call NOW: 615-337-4342
              </a>
            </div>
            <button onClick={() => navigate('/packages')} className="mt-4 sm:mt-6 text-neutral-400 font-bold uppercase tracking-widest text-sm hover:text-crimson-600 transition-colors flex items-center gap-2 mx-auto lg:mx-0 min-h-[44px]" data-testid="button-hero-next">
          ...
          <div className="w-full lg:w-1/2 flex flex-row lg:flex-col items-center justify-center lg:justify-end gap-3 sm:gap-4 relative">
            ...
            <div className="relative sm:-skew-x-12 border border-crimson-900/50 bg-void-800/50 p-1 sm:p-2 shadow-[0_0_20px_rgba(14,165,233,0.1)]">
    ...
    <PageShell scrollable>
      ...
      <div className="flex-1 flex flex-col items-center px-4 sm:px-4 md:px-8 pt-16 sm:pt-20 pb-4">
          ...
          {cards.map((card, i) => (
            <div key={i} className={`group relative w-full h-[220px] sm:h-[300px] md:h-[320px] bg-void-800 sm:-skew-x-12 border border-neutral-800 hover:border-crimson-600 transition-all duration-300 overflow-hidden shadow-2xl cursor-pointer rounded-lg sm:rounded-none ${i % 2 === 1 ? 'sm:mt-8' : ''}`} onClick={() => openBooking(card.pkg)} data-testid={`card-package-${card.pkg}`}>
              ...
              <div className="absolute bottom-0 left-0 w-full p-4 sm:p-6 sm:skew-x-12 flex flex-col justify-end h-full group-hover:-translate-y-2 transition-transform duration-300">
                ...
                <p className="text-[11px] sm:text-sm text-neutral-400 mb-2 sm:mb-3 leading-tight line-clamp-3">{card.desc}</p>
                <p className="text-[11px] sm:text-xs text-neutral-500 mb-2 sm:mb-3">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6 mt-6 sm:mt-10">
          <a href="tel:6153374342" className="text-base sm:text-lg font-display font-bold text-neutral-400 flex items-center gap-2 min-h-[44px]" data-testid="link-packages-phone">
            Call NOW: <span className="text-crimson-600 animate-text-glow">615-337-4342</span>
          </a>
          <button onClick={() => navigate('/sites')} className="text-neutral-500 font-bold uppercase tracking-widest text-sm hover:text-crimson-600 transition-colors flex items-center gap-2 min-h-[44px]" data-testid="button-packages-next">
      ...
      <div className="flex-1 flex flex-col items-center px-4 sm:px-4 md:px-8 pt-16 sm:pt-20 pb-4">
        <div className="w-full max-w-7xl py-4 sm:py-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-4 mb-6 sm:mb-12">
            {SITES.map((site, i) => (
              <div key={i} className="group sm:-skew-x-12 bg-void-800/60 border border-neutral-800 hover:border-crimson-600 transition-all duration-300 p-3 sm:p-4 cursor-default rounded-md sm:rounded-none" data-testid={`text-site-${i}`}>
                <span className="block sm:skew-x-12 text-white font-display text-xs sm:text-sm tracking-wide uppercase group-hover:text-crimson-500 transition-colors">{site}</span>
              </div>
            ))}
          </div>

          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mb-6 sm:mb-12 border-b border-neutral-800 pb-4 sm:pb-6">
            <div>
              <h2 className="text-xl sm:text-3xl md:text-4xl font-display font-bold text-white tracking-tighter uppercase mb-2">
                Ride <span className="text-crimson-600">Protocol</span>
              </h2>
              <p className="text-neutral-500 font-medium tracking-wide text-xs sm:text-base">How your Nashville party bus goes from idea to &ldquo;best night of the trip&rdquo;.</p>
            </div>
          ...
          <div className="relative w-full grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-6 sm:mb-12">
            {[
              ...
              <div key={s.step} className={`group relative w-full h-36 sm:h-48 sm:-skew-x-12 bg-void-800/60 border border-neutral-800 hover:border-crimson-600 hover:bg-void-700 transition-all duration-300 p-3 sm:p-6 flex flex-col justify-between rounded-md sm:rounded-none`} data-testid={`text-step-${s.step}`}>
                <div className="sm:skew-x-12 flex justify-between items-start">
                  ...
                  <iconify-icon icon={s.icon} className="text-lg sm:text-2xl text-neutral-600 group-hover:text-white transition-colors" style={{ strokeWidth: 1.5 }}></iconify-icon>
                </div>
                <div className="sm:skew-x-12">
                  <h3 className="text-sm sm:text-xl font-display font-bold text-white uppercase mb-1">{s.title}</h3>
                  <p className="text-[9px] sm:text-xs text-neutral-500 leading-relaxed">{s.desc}</p>
                </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-6 mb-6 sm:mb-10">
            {[
              ...
              <div key={i} className="group h-20 sm:h-32 sm:-skew-x-12 bg-gradient-to-r from-void-800/60 to-void-900/60 border-l-2 border-neutral-700 hover:border-crimson-600 pl-3 sm:pl-6 flex items-center transition-all cursor-pointer rounded-md sm:rounded-none" data-testid={`text-feature-${['enclosed','vip','route'][i]}`}>
                <div className="sm:skew-x-12">
                  <span className="text-[9px] sm:text-[10px] font-mono text-neutral-500 block mb-1">{f.tag}</span>
                  <h3 className="text-sm sm:text-xl font-bold font-display text-white group-hover:text-crimson-500 transition-colors uppercase">{f.label}</h3>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex items-center justify-center gap-4 sm:gap-6">
            <button onClick={() => navigate('/pricing')} className="text-neutral-500 font-bold uppercase tracking-widest text-sm hover:text-crimson-600 transition-colors flex items-center gap-2 min-h-[44px]" data-testid="button-sites-next">
        ...
        <div className="w-full max-w-4xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-6 mb-6 sm:mb-10">
            {[
              ...
              <div key={i} className="group sm:-skew-x-12 bg-void-800/60 border border-neutral-800 hover:border-crimson-600 transition-all duration-300 p-4 sm:p-6 cursor-pointer rounded-md sm:rounded-none" onClick={() => openBooking('custom')}>
                <div className="sm:skew-x-12">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-6">
            <SkewedCTA onClick={() => openBooking('custom')} label="Book Now" />
            <button onClick={() => navigate('/shuttle')} className="text-neutral-500 font-bold uppercase tracking-widest text-sm hover:text-crimson-600 transition-colors flex items-center gap-2 min-h-[44px]" data-testid="button-pricing-next">
    ...
    <PageShell scrollable>
      ...
      <div className="flex-1 flex items-center justify-center px-4 sm:px-4 md:px-8 pt-16 sm:pt-20 pb-4">
        <div className="w-full max-w-4xl">
          ...
          <div className="sm:-skew-x-12 bg-void-800/60 border border-neutral-800 p-4 sm:p-10 mb-6 sm:mb-12 relative rounded-md sm:rounded-none">
            ...
            <div className="sm:skew-x-12">
              ...
              <p className="text-sm sm:text-base md:text-lg text-neutral-400 font-body leading-relaxed mb-4 sm:mb-8 tracking-wide" data-testid="text-shuttle-description">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-6">
                <a href="tel:6153374342" className="text-lg sm:text-2xl md:text-3xl font-display font-bold text-white flex items-center gap-3 hover:text-crimson-600 transition-colors min-h-[44px]" data-testid="link-shuttle-phone">
                  Call NOW: <span className="text-crimson-600 animate-text-glow">615-337-4342</span>
                </a>
                <button onClick={() => navigate('/faq')} className="text-neutral-500 font-bold uppercase tracking-widest text-sm hover:text-crimson-600 transition-colors flex items-center gap-2 min-h-[44px]" data-testid="button-shuttle-next">
      ...
      <div className="flex-1 flex flex-col items-center px-4 sm:px-4 md:px-8 pt-16 sm:pt-20 pb-4">
        <div className="w-full max-w-4xl py-4 sm:py-8">
          ...
          <div className="space-y-0">
            {FAQ_DATA.map((faq, i) => (
              <div key={i} className="group sm:-skew-x-12 bg-void-800/60 border-t border-b border-neutral-800 hover:border-crimson-600 hover:bg-void-700 transition-all duration-300 p-3 sm:p-6" data-testid={`faq-item-${i}`}>
                <div className="sm:skew-x-12">
                  <div className="flex justify-between items-start mb-1 sm:mb-2">
                    <h2 className="text-xs sm:text-base font-display text-white tracking-wide uppercase pr-3">{faq.q}</h2>
                  ...
                  <p className="text-neutral-500 font-body text-[11px] sm:text-sm tracking-wide">{faq.a}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-6 mt-6 sm:mt-10">
            <SkewedCTA onClick={() => openBooking('custom')} label="Book Now" />
            <button onClick={() => navigate('/contact')} className="text-neutral-500 font-bold uppercase tracking-widest text-sm hover:text-crimson-600 transition-colors flex items-center gap-2 min-h-[44px]" data-testid="button-faq-next">
    ...
    <PageShell scrollable>
      ...
      <div className="flex-1 flex items-center justify-center px-4 sm:px-4 md:px-8 pt-16 sm:pt-20 pb-4">
        <div className="w-full max-w-5xl flex flex-col lg:flex-row gap-4 sm:gap-6 items-stretch">
          <div className="flex-1 sm:-skew-x-12 bg-void-800/60 border border-neutral-800 p-4 sm:p-8 md:p-10 relative rounded-md sm:rounded-none">
            <div className="hidden sm:block absolute top-0 left-0 w-8 h-8 border-t-2 border-l-2 border-crimson-600 -translate-x-1 -translate-y-1"></div>
            <div className="hidden sm:block absolute bottom-0 right-0 w-8 h-8 border-b-2 border-r-2 border-crimson-600 translate-x-1 translate-y-1"></div>
            <div className="sm:skew-x-12">
              ...
              <h1 className="text-xl sm:text-3xl font-display font-bold text-white tracking-tighter uppercase mb-4 sm:mb-6">
              ...
              {submitted ? (
                <div className="text-center py-6 sm:py-8" data-testid="contact-success">
                  ...
                  <h3 className="text-xl sm:text-2xl font-display font-bold text-white uppercase tracking-widest mb-2">Message Sent!</h3>
                  <p className="text-neutral-500 font-medium">We'll get back to you soon.</p>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-3">
                  <input type="text" placeholder="Your Name" required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full px-3 sm:px-4 py-3 bg-void-900 border border-neutral-700 focus:border-crimson-600 outline-none font-body text-white placeholder:text-neutral-600 tracking-wide text-sm sm:text-base min-h-[44px] rounded-sm" data-testid="input-contact-name" />
                  <input type="email" placeholder="Email Address" required value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="w-full px-3 sm:px-4 py-3 bg-void-900 border border-neutral-700 focus:border-crimson-600 outline-none font-body text-white placeholder:text-neutral-600 tracking-wide text-sm sm:text-base min-h-[44px] rounded-sm" data-testid="input-contact-email" />
                  <input type="tel" placeholder="Phone Number" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className="w-full px-3 sm:px-4 py-3 bg-void-900 border border-neutral-700 focus:border-crimson-600 outline-none font-body text-white placeholder:text-neutral-600 tracking-wide text-sm sm:text-base min-h-[44px] rounded-sm" data-testid="input-contact-phone" />
          ...
          <div className="w-full lg:w-72 sm:-skew-x-12 bg-void-800/60 border border-neutral-800 p-5 sm:p-8 flex flex-col justify-center items-center relative rounded-md sm:rounded-none">
            <div className="hidden sm:block absolute top-0 left-0 w-6 h-6 border-t-2 border-l-2 border-crimson-600 -translate-x-0.5 -translate-y-0.5"></div>
            <div className="hidden sm:block absolute bottom-0 right-0 w-6 h-6 border-b-2 border-r-2 border-crimson-600 translate-x-0.5 translate-y-0.5"></div>
            <div className="sm:skew-x-12 text-center">
              ...
              <a href="tel:6153374342" className="text-crimson-600 font-bold text-xl sm:text-2xl mb-3 sm:mb-4 animate-text-glow block hover:text-white transition-colors min-h-[44px] flex items-center justify-center" data-testid="link-contact-phone">615-337-4342</a>
