...
function NavBar({ openBooking, currentPage }: { openBooking: (pkg: string) => void; currentPage: string }) {
  ...
  return (
    <nav className="fixed top-0 w-full z-50 bg-white/70 backdrop-blur-sm border-b border-neutral-200">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8">
        <div className="flex justify-between h-14 sm:h-16 items-center">
          ...
          <div className="hidden lg:flex space-x-1 items-center">
            {navItems.map(item => (
              <button key={item.path} onClick={() => navigate(item.path)} className={`font-display font-bold uppercase tracking-wider text-xs px-3 py-1.5 transition-all duration-300 ${currentPage === item.path ? 'text-crimson-600 border-b-2 border-crimson-600' : 'text-neutral-600 hover:text-neutral-900 border-b-2 border-transparent hover:border-neutral-300'}`} data-testid={`link-${item.label.toLowerCase().replace(/[^a-z]/g, '')}`}>{item.label}</button>
            ))}
          ...
          <button className="lg:hidden text-neutral-800 text-3xl min-w-[44px] min-h-[44px] flex items-center justify-center" onClick={() => setMobileMenuOpen(!mobileMenuOpen)} data-testid="button-mobile-menu">
            {mobileMenuOpen ? '\u2715' : '\u2630'}
          </button>
        </div>
      </div>
      {mobileMenuOpen && (
        <div className="lg:hidden bg-white/95 backdrop-blur-md border-t border-neutral-200 px-4 py-3 space-y-1 max-h-[80vh] overflow-y-auto">
          {navItems.map(item => (
            <button key={item.path} onClick={() => { navigate(item.path); setMobileMenuOpen(false); }} className={`block w-full text-left font-display font-bold uppercase tracking-wider text-sm py-3 px-3 transition-all duration-200 min-h-[44px] ${currentPage === item.path ? 'text-crimson-600 border-l-2 border-crimson-600 bg-sky-50' : 'text-neutral-700 hover:text-neutral-900 hover:bg-neutral-100'}`}>{item.label}</button>
          ))}
...
function Footer() {
  return (
    <footer className="w-full bg-white/80 border-t border-neutral-200 py-2 px-3 sm:px-6 text-center z-20 shrink-0">
...
function PageShell({ children, scrollable = false }: { children: React.ReactNode; scrollable?: boolean }) {
  return (
    <div className="relative overflow-hidden bg-white" style={{ margin: 0, padding: 0, width: '100vw', height: '100vh' }}>
      ...
      <div className="fixed inset-0 z-0 bg-gradient-to-b from-white/20 via-transparent to-white/30 pointer-events-none"></div>
      ...
      <div className="flex-1 flex items-center justify-center px-4 sm:px-4 md:px-8 pt-16 sm:pt-20 pb-4">
        <div className="w-full max-w-7xl flex flex-col lg:flex-row items-center justify-between gap-6 sm:gap-8">
          <div className="w-full lg:w-1/2 text-center lg:text-left">
            ...
            <p className="text-sm sm:text-base md:text-lg text-neutral-600 font-body mb-4 sm:mb-8 max-w-xl mx-auto lg:mx-0 tracking-wide" data-testid="text-hero-description">
            ...
            <button onClick={() => navigate('/packages')} className="mt-4 sm:mt-6 text-neutral-500 font-bold uppercase tracking-widest text-sm hover:text-crimson-600 transition-colors flex items-center gap-2 mx-auto lg:mx-0 min-h-[44px]" data-testid="button-hero-next">
    ...
    <PageShell scrollable>
      ...
      <div className="flex-1 flex flex-col items-center px-4 sm:px-4 md:px-8 pt-16 sm:pt-20 pb-4">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6 mt-6 sm:mt-10">
          <a href="tel:6153374342" className="text-base sm:text-lg font-display font-bold text-neutral-600 flex items-center gap-2 min-h-[44px]" data-testid="link-packages-phone">
      ...
      <div className="flex-1 flex flex-col items-center px-4 sm:px-4 md:px-8 pt-16 sm:pt-20 pb-4">
        <div className="w-full max-w-7xl py-4 sm:py-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-4 mb-6 sm:mb-12">
            {SITES.map((site, i) => (
              <div key={i} className="group sm:-skew-x-12 bg-white/60 border border-neutral-200 hover:border-crimson-600 transition-all duration-300 p-3 sm:p-4 cursor-default rounded-md sm:rounded-none shadow-sm" data-testid={`text-site-${i}`}>
                <span className="block sm:skew-x-12 text-neutral-800 font-display text-xs sm:text-sm tracking-wide uppercase group-hover:text-crimson-500 transition-colors">{site}</span>
              </div>
            ))}
          </div>
          ...
          <div className="relative w-full grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-6 sm:mb-12">
            {[
              ...
              <div key={s.step} className={`group relative w-full h-36 sm:h-48 sm:-skew-x-12 bg-white/60 border border-neutral-200 hover:border-crimson-600 hover:bg-sky-50 transition-all duration-300 p-3 sm:p-6 flex flex-col justify-between rounded-md sm:rounded-none shadow-sm`} data-testid={`text-step-${s.step}`}>
                <div className="sm:skew-x-12 flex justify-between items-start">
                  ...
                  <iconify-icon icon={s.icon} className="text-lg sm:text-2xl text-neutral-400 group-hover:text-crimson-600 transition-colors" style={{ strokeWidth: 1.5 }}></iconify-icon>
                </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-6 mb-6 sm:mb-10">
            {[
              ...
              <div key={i} className="group h-20 sm:h-32 sm:-skew-x-12 bg-white/60 border-l-2 border-neutral-300 hover:border-crimson-600 pl-3 sm:pl-6 flex items-center transition-all cursor-pointer rounded-md sm:rounded-none shadow-sm" data-testid={`text-feature-${['enclosed','vip','route'][i]}`}>
                <div className="sm:skew-x-12">
                  <span className="text-[9px] sm:text-[10px] font-mono text-neutral-400 block mb-1">{f.tag}</span>
        ...
        <div className="w-full max-w-4xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-6 mb-6 sm:mb-10">
            {[
              ...
              <div key={i} className="group sm:-skew-x-12 bg-white/60 border border-neutral-200 hover:border-crimson-600 transition-all duration-300 p-4 sm:p-6 cursor-pointer rounded-md sm:rounded-none shadow-sm" onClick={() => openBooking('custom')}>
                <div className="sm:skew-x-12">
    ...
    <PageShell scrollable>
      ...
      <div className="flex-1 flex items-center justify-center px-4 sm:px-4 md:px-8 pt-16 sm:pt-20 pb-4">
        <div className="w-full max-w-4xl">
          ...
          <div className="sm:-skew-x-12 bg-white/60 border border-neutral-200 p-4 sm:p-10 mb-6 sm:mb-12 relative rounded-md sm:rounded-none shadow-sm">
            ...
            <div className="sm:skew-x-12">
              ...
              <p className="text-sm sm:text-base md:text-lg text-neutral-600 font-body leading-relaxed mb-4 sm:mb-8 tracking-wide" data-testid="text-shuttle-description">
      ...
      <div className="flex-1 flex flex-col items-center px-4 sm:px-4 md:px-8 pt-16 sm:pt-20 pb-4">
        <div className="w-full max-w-4xl py-4 sm:py-8">
          ...
          <div className="space-y-0">
            {FAQ_DATA.map((faq, i) => (
              <div key={i} className="group sm:-skew-x-12 bg-white/60 border-t border-b border-neutral-200 hover:border-crimson-600 hover:bg-sky-50 transition-all duration-300 p-3 sm:p-6" data-testid={`faq-item-${i}`}>
                <div className="sm:skew-x-12">
    ...
    <PageShell scrollable>
      ...
      <div className="flex-1 flex items-center justify-center px-4 sm:px-4 md:px-8 pt-16 sm:pt-20 pb-4">
        <div className="w-full max-w-5xl flex flex-col lg:flex-row gap-4 sm:gap-6 items-stretch">
          <div className="flex-1 sm:-skew-x-12 bg-white/60 border border-neutral-200 p-4 sm:p-8 md:p-10 relative rounded-md sm:rounded-none shadow-sm">
            <div className="hidden sm:block absolute top-0 left-0 w-8 h-8 border-t-2 border-l-2 border-crimson-600 -translate-x-1 -translate-y-1"></div>
            <div className="hidden sm:block absolute bottom-0 right-0 w-8 h-8 border-b-2 border-r-2 border-crimson-600 translate-x-1 translate-y-1"></div>
            <div className="sm:skew-x-12">
          ...
          <div className="w-full lg:w-72 sm:-skew-x-12 bg-white/60 border border-neutral-200 p-5 sm:p-8 flex flex-col justify-center items-center relative rounded-md sm:rounded-none shadow-sm">
            <div className="hidden sm:block absolute top-0 left-0 w-6 h-6 border-t-2 border-l-2 border-crimson-600 -translate-x-0.5 -translate-y-0.5"></div>
            <div className="hidden sm:block absolute bottom-0 right-0 w-6 h-6 border-b-2 border-r-2 border-crimson-600 translate-x-0.5 translate-y-0.5"></div>
